...
riot.tag2('page-viewjournal', '<section class="section"> <div class="container"> <section class="section"> <h3 class="title">Journal: {journal.Title}</h3> <p> <raw class="markdown" content="{journal.HtmlContent}"></raw> </p> </section> <section class="section"> <div class="box" each="{entry in entries.Entries}" onclick="{onentry}" style="cursor:pointer;"> <article class="media"> <div class="media-content"> <div class="content"> <p> <h3 class="title">{entry.Title}</h3> <small>@{user.Username}</small> <small>{entry.Date}</small> <raw class="markdown" content="{entry.preview}"></raw> <br> <p if="{entry.Content.length > 2000}"><span>Read more</span></p> <span each="{tag in parent.Tags}">{tag}</span> </p> </div> </div> </article> </div> <button class="button is-primary" if="{entries.HasNext}" onclick="{loadMore}">Load more</button> </section> </div> </section>', '', '', function(opts) {
var self = this;
